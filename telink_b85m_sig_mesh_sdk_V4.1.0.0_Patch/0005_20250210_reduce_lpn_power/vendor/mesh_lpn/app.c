...
void main_loop ()
{
    ...
    if(is_lpn_support_and_en){
        ...
        if(is_friend_ship_link_ok_lpn() && (CMD_CTL_POLL == fri_ship_proc_lpn.last_op)){
            fri_ship_proc_lpn.last_op = 0;
            return; // if just send friend poll, not run later loop to save power.
        }
	}
