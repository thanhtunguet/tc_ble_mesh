 ...
 */
public class FastProvisionDeviceAdapter extends BaseRecyclerViewAdapter<FastProvisionDeviceAdapter.ViewHolder> {
    private List<NetworkingDevice> mDevices;
    private Context mContext;
    boolean processing = false;

    public FastProvisionDeviceAdapter(Context context, List<NetworkingDevice> devices) {
        mContext = context;
        mDevices = devices;
    }
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(mContext).inflate(R.layout.item_fast_provision, parent, false);
...
//        holder.pb_provision.setIndeterminate(false);
        ...
        if (device.state == NetworkingState.IDLE) {
            ...
            if (device.isProcessing()) {
                ...
                } else if (device.state == NetworkingState.PROVISION_SUCCESS || device.nodeInfo.bound) {
        ...
        holder.tv_log_latest.setText(lastLog.logMessage);
